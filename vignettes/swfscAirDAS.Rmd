...
Part of the check is to check that all sighting events use expected species codes. The package includes a [default SpCodes file](https://github.com/SWFSC/swfscAirDAS/blob/main/inst/extdata/SpCodesAirDAS.DAT); however, `airdas_check` does have an argument `sp.codes.file` if you need to use a custom SpCodes file.
...
One other processing note is that the 'DateTime' column in the `airdas_read` output has a time zone of "". This means that when this 'DateTime' column is printed (or any other DateTime object without a timezone), RStudio will print these values with the local timezone. You can confirm the timezone by running `attr(y.read$DateTime, "tzone")`. If you need to do conversions based on time zone, then use `force_tz` and `with_tz` from the `lubridate` package. See [here](https://github.com/SWFSC/swfscAirDAS/issues/7) for more discussion
