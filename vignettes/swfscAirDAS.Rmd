...
Part of the check is to check that all sighting events use expected species codes.  The package includes a [default SpCodes file](https://github.com/SWFSC/swfscAirDAS/blob/main/inst/SpCodesAirDAS.DAT); however, `airdas_check` does have an argument `sp.codes.file` if you need to use a custom SpCodes file.
