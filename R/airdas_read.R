#' Read AirDAS file(s)
#'
#' Read one or more fixed-width aerial survey DAS text file(s) generated by
#' TURTLEP, or another AirDAS program, into a data frame, where each line is
...
#'
#' @details 
#' Reads/parses aerial survey DAS data into columns of a data frame. If `file`
#' contains multiple filenames, then the individual data frames will be combined
#' using [base::rbind()]
#'
#' See [airdas_format_pdf()] for information about AirDAS format requirements
#' for the specific file types (programs)
#'   
#' @returns
#' An `airdas_dfr` object, which is also a data frame, with AirDAS data read
#' into columns. The data are read into the data frame as characters, with the
...
#' | DateTime  | POSIXct   | combination of 'Date' and 'Time' columns, with time zone of `""` |
...
.airdas_read_phocoena <- function(file, skip, ...) {
  ...
  # Process specifics
  x$DateTime <- strptime(paste(x$Date, x$Time), "%m/%d/%y %H:%M:%S", tz = "")
  x$EffortDot <- NA
...
.airdas_read_survey <- function(file, skip, ...) {
...
.airdas_read_turtle <- function(file, skip, ...) {
  # Start and end (inclusive) column indices
